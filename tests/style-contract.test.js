...
test('semantic theme contracts exist for migrated controls', () => {
    ...
    for (const token of ['--ds-scrim', '--ds-keypad-shadow', '--ds-confetti-1', '--ds-blur-surface', '--ds-layer-header', '--ds-layer-update', '--ds-layer-table-corner', '--ds-divider']) {
        assert.match(tokens, new RegExp(token));
    }
    assert.match(tokens, /--ds-scrim:\s*rgb\(15 23 42 \/ 0\.14\)/);
    ...
    assert.match(tokens, /\.ds-input\s*\{[\s\S]*?border:\s*1px solid var\(--ds-input-border\)/);
    assert.match(tokens, /\.selection-status\s*\{[\s\S]*?height:\s*4rem;[\s\S]*?align-items:\s*center;/);
});
