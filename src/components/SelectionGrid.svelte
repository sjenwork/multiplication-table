<script lang="ts">
  ...
  let gesture: {
    ...
    touched: Set<string>; selecting: boolean; active: boolean; timer: ReturnType<typeof setTimeout>;
  } | null = null;
  ...
  function beginPointer(event: PointerEvent) {
    ...
    gesture = {
      ...
      touched: new Set(), selecting: false, active: false, timer: setTimeout(() => activateGesture(), LONG_PRESS_MS),
    };
...
<table bind:this={grid} class="selection-grid" role="grid" aria-label="九九乘法選題表"
  ...
  <tbody>
    {#each factors as row}
      ...
      <tr>
        <th scope="row" class:active={rowSelected} class="row-heading">
        ...
        {#each factors as column}
          ...
          <td data-question={question.key} onpointerenter={enterCell} class:selected={dragChanges.has(question.key) ? dragChanges.get(question.key) : selectedSet.has(question.key)}>
...
<style>
  ...
  .corner-divider { display: block; min-height: 1.1rem; color: var(--ds-text-muted); font-size: 0.9rem; line-height: 1; }
  .column-heading button { color: var(--ds-factor-two); }
  .row-heading button { color: var(--ds-factor-one); }
  th.active { background: var(--ds-brand-soft); }
  th.active button { border-color: var(--ds-brand-strong); }
